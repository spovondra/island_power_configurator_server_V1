...
public class ProjectLocationService {
    ...
    private static final String WEATHER_API_URL = "https://re.jrc.ec.europa.eu/api/v5_3/seriescalc?lat=%s&lon=%s&raddatabase=PVGIS-SARAH3&outputformat=json&&startyear=2023&endyear=2023";
    ...
    public void processLocationData(String projectId, String userId, double latitude, double longitude, int angle, int aspect, boolean useOptimalValues) {
        ...
        if (useOptimalValues) {
            ...
            List<Site.MonthlyIrradiance> monthlyHI_dList = calculatePVGISData(latitude, longitude, optimalValues.optimalAngle(), optimalValues.optimalAspect());
            ...
            List<Site.MonthlyIrradiance> monthlyHI_dList = calculatePVGISData(latitude, longitude, angle, aspect);
    ...
    public List<Site.MonthlyIrradiance> calculatePVGISData(double latitude, double longitude, double angle, double aspect) {
        List<Site.MonthlyIrradiance> monthlyIrradianceList = new ArrayList<>();
        try {
            ...
            for (JsonNode monthNode : monthlyDataNode) {
                Site.MonthlyIrradiance irradiance = new Site.MonthlyIrradiance();
                int month = monthNode.path("month").asInt();
                double HI_d = monthNode.path("H(i)_d").asDouble();
                irradiance.setMonth(month);
                irradiance.setIrradiance(HI_d);
                monthlyIrradianceList.add(irradiance);
            }
        } catch (Exception e) {
